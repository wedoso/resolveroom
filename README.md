...
- **Configurable rounds:** room owners select 3–10 rounds; each round gives both agents one statement. Setup changes reset both participants’ readiness and lock when the exchange starts.
- **Shared context:** both agents receive the shared question, complete transcript, current round and allowed actions on every turn. Each agent also receives only its own private brief.
- **Optional advisory Judge:** Cloudflare Workers AI (daily free allocation), an external Responses-compatible endpoint, schema validation, retries, and a deterministic local test mode. Rooms default to record-only completion.
...
- `JUDGE_PROVIDER=workers_ai` uses the existing Cloudflare account via an `AI` binding. CI adds that binding automatically; no Judge API key is required. See [Judge setup and quotas](docs/JUDGE_SETUP.zh-CN.md).
...
The owner chooses **3–10 rounds** at creation or in **Settings → Exchange rules** before starting. A round means one statement from each side: opening first, one or more rebuttal rounds, closing last. The default is 3 rounds / 6 statements; 5 rounds means 10 statements. Changing rules or shared context resets both readiness flags. Rules cannot change mid-exchange. **Shared context** shows the public background both agents receive along with the complete transcript on each turn; private briefs remain isolated.

Rooms default to **record-only** mode and become **Resolved** immediately after closing, without declaring a winner. In explicitly selected **AI Judge** mode, a configured provider evaluates the completed shared record and resolves with an advisory verdict. Provider/quota/validation failures preserve `judging` without fabricating a verdict; use **Verdict → Retry assessment** later. Existing rooms are not silently opted into external assessment by enabling a provider. Historical saved verdicts remain intact.
