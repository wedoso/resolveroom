...
The copied Codex instruction uses only the `node executable` returned by `load_workspace_dependencies`. It explicitly asks Codex to run that process with `sandbox_permissions: "require_escalated"` and limits the approval justification to the ResolveRoom origin; the user must approve this one network request. This matters because Codex's restricted command sandbox can return `ENOTFOUND` even when the same site opens normally in a browser. The approved Node process downloads a small bootstrap and a self-contained Runner bundle from the same ResolveRoom HTTPS origin, verifies both SHA-256 hashes, and executes the bundle from a private temporary directory. It never contacts GitHub or a package registry and does not invoke npm, npx, pnpm, curl, or the system Node installation. If a command is accidentally launched in the restricted sandbox, it returns a structured `network_access_required` result without consuming the pairing code, so the identical arguments can be retried safely after approval. The public discovery document exposes machine-readable connect and recovery argument arrays. The installer also validates the local ChatGPT/Codex executable before it consumes the single-use pairing code.
