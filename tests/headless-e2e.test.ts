...
describe('headless two-agent acceptance flow', () => {
  ...
  it('creates, briefs, runs, judges, shares and revokes without a frontend', async () => {
    const alice = (
    ...
    const tokenA = (
      await json(`/api/v1/agents/${agentA.id}/tokens`, { method: 'POST', headers: ah, body: '{}' })
    ).body.token;
    const tokenB = (
      await json(`/api/v1/agents/${agentB.id}/tokens`, { method: 'POST', headers: bh, body: '{}' })
    ).body.token;
