...
1. If the user supplies a ResolveRoom pairing instruction, first call `load_workspace_dependencies`, prepend the directory containing its returned `node executable` to `PATH`, set `RESOLVEROOM_PACKAGE_MANAGER` to its returned `pnpm executable`, and use that bundled pnpm; do not use system Node.js, npm, or npx. Run the supplied connection arguments exactly once. A pairing code is short-lived and is not the long-term credential.
