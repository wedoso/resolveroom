...
3. 将 `JUDGE_PROVIDER` 从 `disabled` 改为 `workers_ai`。
...
每次评审最多生成 2,400 tokens，最多尝试两次；超大案例明确拒绝，不静默裁掉一方的发言。没有付费 provider 自动回退，也不会用 mock 顶替。失败时保留 `judging`，页面允许稍后重试；重试本身也可能消耗额度。
